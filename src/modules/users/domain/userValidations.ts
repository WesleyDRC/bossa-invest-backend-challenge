...
export function mustAttentionIn(
  ...
  if (!hasCorrectEmailFormatRequired(email)) return userConstants.INVALID_EMAIL_ERROR;

  if (!hasMinPasswordLengthRequired(password)) return userConstants.INVALID_PASSWORD_ERROR;

  if (!isCorrectPassword(password, confirmPassword)) return userConstants.INVALID_CORRECT_PASSWORD;
}
