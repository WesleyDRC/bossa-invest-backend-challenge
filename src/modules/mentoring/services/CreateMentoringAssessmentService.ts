...
export class CreateMentoringAssessmentService {
	...
	async execute({
		...
		const alreadyExistsAssessment = await this.mentoringAssessmentRepository.findBySessionId(sessionId)

		if (alreadyExistsAssessment) {
			throw new AppError(mentoringConstants.ALREADY_EXISTS_ASSESSMENT, 409)
		}

		const userParticipatedMentoring = this.userParticipatedMentoring(menteeId, foundMentoringSession)
	...
	private userParticipatedMentoring(menteeId: string, mentoringSession: IMentoringSession): boolean {
