export function formatDateTime(scheduledAt: Date, hourStart: string, hourEnd: string) {
	...
	const startDateTimeUTC = new Date(localStartDateTime.getTime() - localStartDateTime.getTimezoneOffset() * 60000);
	const endDateTimeUTC = new Date(localEndDateTime.getTime() - localEndDateTime.getTimezoneOffset() * 60000);
